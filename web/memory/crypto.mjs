import { keccak256, toUtf8Bytes } from '../vendor/ethers.min.js';
// Exact serializer and digest domain from MASTER: retaining their byte encoding
// keeps existing AES-GCM authenticated headers and memory seals interoperable.
function stable(v){if(Array.isArray(v))return '['+v.map(stable).join(',')+']';if(v&&typeof v==='object')return '{'+Object.keys(v).sort().map(k=>JSON.stringify(k)+':'+stable(v[k])).join(',')+'}';return JSON.stringify(v);}
const fingerprint = value => keccak256(toUtf8Bytes(stable(value)));
