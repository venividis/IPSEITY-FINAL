...
import { Interface, formatEther, getAddress } from '../vendor/ethers.min.js';
import { sameAuthority, boundedJSON } from './host.mjs';
import { parseJournalPacket, prepareJournal, IPSEITY_JOURNAL_MODULE_KEY, IPSEITY_JOURNAL_STATE_SCHEMA } from './journal.mjs';
...
export class RegistryAdapter {
 constructor(wallet, config){this.wallet=wallet;this.config={...config,registry:getAddress(config.registry)};this.request=value=>wallet.raw.request(value);this.snapshot=null;this.prepared=new WeakMap();}
 async context(){
 ...
 async installation(moduleKey,context=this.snapshot){
  if(!context)context=await this.context();const result=await sdk.readCall(this.request,context.snapshot,context.registry,sdk.TOKEN_MODULE_ABI,'installation',[context.tokenId,moduleKey]);
 ...
 async refresh({catalogCursor=0,moduleCursor=0,historyCursor=0}={}){
  ...
  const result=await sdk.readCall(this.request,c.snapshot,c.releases,sdk.RELEASE_ABI,'catalog',[catalogCursor,16]);
  const count=await sdk.readCall(this.request,c.snapshot,c.releases,sdk.RELEASE_ABI,'releaseCount');
 ...
 async journalPage({cursor=0,limit=16}={}){
  if(!Number.isSafeInteger(cursor)||cursor<0||!Number.isSafeInteger(limit)||limit<1||limit>16)throw Error('Invalid journal page.');
  const c=await this.context(),[count]=await sdk.readCall(this.request,c.snapshot,c.stateStore,sdk.STATE_ABI,'countOf',[c.tokenId,IPSEITY_JOURNAL_MODULE_KEY]);
  if(BigInt(cursor)>count)throw Error('Journal cursor exceeds the current history.');
  const [ids,next]=await sdk.readCall(this.request,c.snapshot,c.stateStore,sdk.STATE_ABI,'historyOf',[c.tokenId,IPSEITY_JOURNAL_MODULE_KEY,cursor,limit]);
  const expected=Number(count-BigInt(cursor)>BigInt(limit)?BigInt(limit):count-BigInt(cursor));
  if(ids.length!==expected||next!==BigInt(cursor+ids.length)||new Set(ids).size!==ids.length)throw Error('Invalid journal history page.');
  const entries=[];
  for(const stateId of ids){
   const saved=await sdk.recoverState({request:this.request,stateStore:c.stateStore,stateId,chainId:c.chainId,snapshot:c.snapshot,collection:c.collection,tokenId:c.tokenId,moduleKey:IPSEITY_JOURNAL_MODULE_KEY,maxExpandedBytes:16384});
   if(saved.record.schema!==IPSEITY_JOURNAL_STATE_SCHEMA)throw Error('Unexpected journal state schema.');
   const text=new TextDecoder('utf-8',{fatal:true}).decode(saved.bytes);let encrypted=false;
   try{parseJournalPacket(text);encrypted=true;}catch{}
   entries.push({stateId,text,encrypted,epoch:saved.record.epoch,createdAt:saved.record.createdAt,index:saved.record.index});
  }
  await sdk.assertSnapshot(this.request,c.snapshot);await this.fresh(c.identity);
  return{entries,next:Number(next),count:count.toString(),snapshot:c.snapshot};
 }
 async prepare(intent){
  this.wallet.plan=null;
  const c=await this.context();
  if(!sameAuthority(c.identity,intent.identity))throw Error('The review belongs to another NFT or custody epoch.');
  ...
  if(intent.kind==='journal'){
   ...
   }else if(intent.privacyMode==='public')await prepareJournal({mode:'public',text:intent.text});
   else throw Error('Choose public or encrypted journal publication.');
   recipe=sdk.encodeStageState(c,{moduleKey:IPSEITY_JOURNAL_MODULE_KEY,stateSchema:IPSEITY_JOURNAL_STATE_SCHEMA,bytes:new TextEncoder().encode(intent.text)});
  }
  let prepared;
  if(intent.kind==='module-proposal'){
   await this.fresh(intent.identity,intent.releaseId,intent.moduleKey);
   prepared=await this.wallet.prepare({target:intent.proposal.to,value:formatEther(BigInt(intent.proposal.value)),data:intent.proposal.data});
  }else{
   for(const field of ['collection','tokenId','account','owner','epoch','root','actionNonce'])if(String(recipe?.preconditions?.[field]).toLowerCase()!==String(c[field]).toLowerCase())throw Error('The reviewed module context changed: '+field+'.');
   const outer=new Interface(sdk.ACCOUNT_ABI).parseTransaction({data:recipe.data});
   if(outer.name!=='execute'||outer.args.length!==4||outer.args[3]!==0n||recipe.to.toLowerCase()!==this.wallet.account.toLowerCase()||BigInt(recipe.value)!==0n||outer.args[0].toLowerCase()!==c.registry.toLowerCase())throw Error('The SDK recipe does not target this NFT Reach and module registry.');
   prepared=await this.wallet.prepare({target:outer.args[0],value:formatEther(outer.args[1]),data:outer.args[2]});
  }
  this.prepared.set(prepared,c);return prepared;
 }
 async send(prepared,intent){
  if(this.wallet.plan!==prepared||!this.prepared.has(prepared))throw Error('The wallet review changed. Prepare again.');
  const reviewed=this.prepared.get(prepared);this.prepared.delete(prepared);
  await sdk.assertCurrentContext({request:this.request,context:reviewed});
  ...
  if(intent.kind==='stageState'||intent.kind==='journal'){
   ...
   for(const log of receipt.logs??[]){if(log.address.toLowerCase()!==c.stateStore.toLowerCase())continue;try{const event=abi.parseLog(log);if(event?.name==='StateStaged'&&event.args.schema===(intent.kind==='journal'?IPSEITY_JOURNAL_STATE_SCHEMA:intent.stateSchema))stagedStateId=event.args.id;}catch{}}
  ...
  return{...receipt,...(intent.kind==='journal'?{journalStateId:stagedStateId}:{stagedStateId})};
 }
}
