...
export async function buildModuleSDK({output=path.join(root,'packages/modules/dist')}={}) {
  ...
  const inputs=[...Object.keys(result.metafile.inputs),'tools/build-module-sdk.mjs','packages/modules/package.json'];
...
if(process.argv[1]&&path.resolve(process.argv[1])===path.resolve(import.meta.filename)) {
  if(process.argv.length>2)throw Error('Usage: node tools/build-module-sdk.mjs');
  const result=await buildModuleSDK();console.log('Built portable @ipseity/modules: '+result.manifest.files['index.mjs'].byteLength+' bytes; no runtime dependencies.');
}
