...
/*════════════ one collection, one chain, one numbering ════════════
  ...
  Ethereum issues the complete #1..#4096 edition. Testnets rehearse the same
  range but are not part of production, and there are no chain bands.
*/
head("Ethereum owns the complete edition");
{
  eq("the edition contains 4096 tokens", decUint(await c.read(nft, "COLLECTION()")), 4096n);
  eq("the mint ceiling is the complete edition", decUint(await c.read(nft, "MAX_SUPPLY()")), 4096n);
  eq("the first id is one", decUint(await c.read(nft, "FIRST_ID()")), 1n);
  eq("the last id is 4096", decUint(await c.read(nft, "LAST_ID()")), 4096n);
  eq("the first minted token is #1",
     decUint(await c.read(nft, "tokenByIndex(uint256)", [0])), 1n);
  const foreign = await Chain.open({ chainId: 8453 });
  let refused = false;
  try {
    await foreign.deploy(A.Ipseity.bytecode,
      encodeAddressArg(renderer) + encodeAddressArg(acctImpl) + encodeAddressArg(gripImpl));
  } catch { refused = true; }
  ok("a non-Ethereum production chain is refused", refused);
  console.log("      one Ethereum collection, ids 1..4096 — no bands and no bridge");
}
