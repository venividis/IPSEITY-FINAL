...
import {atomicDirectory} from './modules-files.mjs';
...
export async function buildWorkbench({output=path.join(root,'dist/modules')}={}) {
  ...
  const bytes=Buffer.from(html),packed=gzipSync(bytes,{level:9,mtime:0});
  ...
  const inputs=[...Object.keys(result.metafile.inputs),'web/modules/index.html','web/modules/workbench.css','tools/modules-build-workbench.mjs'];
  const manifest={schema:'anima.module-workbench/1',compression:'raw',byteLength:bytes.length,sha256:hash(bytes),chunks,packed:{file:'packed.bin',compression:'gzip',byteLength:packed.length,sha256:hash(packed)},
  ...
  if(output!==null)await atomicDirectory(output,async stage=>{
    ...
    fs.writeFileSync(path.join(stage,'packed.bin'),packed);fs.writeFileSync(path.join(stage,'index.html'),bytes);fs.writeFileSync(path.join(stage,'manifest.json'),JSON.stringify(manifest,null,2)+'\n');
  });
  return {html,bytes,packed,manifest};
}
if(process.argv[1]&&path.resolve(process.argv[1])===path.resolve(import.meta.filename)) {
  if(process.argv.length>2)throw Error('Usage: node tools/modules-build-workbench.mjs');
