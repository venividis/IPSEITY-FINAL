...
const NAMES = ['ModuleArchiveFactory','ExtensionReleaseRegistry','TokenModuleRegistry','ModuleStateStore',
  'ChunkedCartridgeRegistry','ArtifactBinding','AppChunk','OnchainApp','OnchainAppDirectory','ModuleWorkbench','ModulePortal'];
...
export function validateModuleDeploymentConfig(input) {
  const keys=['chainId','deployer','startingNonce','collection','premises'];
  ...
  integer(input.chainId,1,Number.MAX_SAFE_INTEGER,'chain ID');
  return {chainId:input.chainId,deployer:address(input.deployer,'deployer'),startingNonce:integer(input.startingNonce,0,Number.MAX_SAFE_INTEGER-100000,'starting nonce'),collection:address(input.collection,'existing IPSEITY collection'),premises:address(input.premises,'existing Premises')};
}
...
export function readWorkbenchBuild(manifestPath, projectRoot=root) {
  ...
  const packed=fs.readFileSync(inside(base,manifest.packed?.file));
  if(manifest.packed.compression!=='gzip'||packed.length!==manifest.packed.byteLength||digest(packed)!==manifest.packed.sha256||!gunzipSync(packed,{maxOutputLength:1048576}).equals(bytes))throw Error('Packed workbench differs from raw document');
  return {bytes,packed,manifest,manifestSha256:digest(manifestBytes)};
}
...
export async function planModuleDeployment(input, {artifacts,workbench,verification={scope:'caller-supplied build inputs'}}) {
  ...
  const packed=Buffer.from(workbench.packed??gzipSync(bytes,{level:9,mtime:0}));
  if(!gunzipSync(packed,{maxOutputLength:1048576}).equals(bytes))throw Error('Packed workbench differs from raw bytes');
  const packedArchive=await planArchiveDeployment({archive:packed,publisher:config.deployer,chainId:config.chainId,startNonce:config.startingNonce+steps.length,factory,factoryNonce:archive.nextFactoryNonce,appChunkArtifact:artifacts.AppChunk});
  for(const step of packedArchive.steps)steps.push({...step,dataHash:keccak256(step.data),...(step.to===null?{initcodeBytes:(step.data.length-2)/2}:{preconditions:{factory,expectedFactoryNonce:archive.nextFactoryNonce,onConflict:'Stop and regenerate the remaining plan.'}})});
  const packedChunks=packedArchive.chunkReferences.map(hash=>packedArchive.chunks.find(chunk=>chunk.hash===hash).address);
  const packedAddress=await internal('OnchainApp',factory,archive.nextFactoryNonce,[packedChunks,packedArchive.archiveHash],'PackedWorkbenchArchive');
  if(packedAddress.toLowerCase()!==packedArchive.archiveAddress.toLowerCase())throw Error('Packed archive address prediction differs');
  await deploy('ModulePortal',[config.premises,modules.ModuleWorkbench,modules.PackedWorkbenchArchive,cartridges]);
  steps.at(-1).preconditions={originalPremises:config.premises,collection:config.collection,workbench:modules.ModuleWorkbench,packedArchive:modules.PackedWorkbenchArchive,packedSha256:digest(packed),expandedSha256:digest(bytes),onConflict:'Verify original Premises.HUB and both immutable archive commitments before signing.'};
  const result={schema:'ipseity.module-system-deployment/1',status:'unsigned-companion-plan',config,modules,artifacts:bindings,verification,
    ...
    archive,packedArchive,internalCreations,steps,nextNonce:config.startingNonce+steps.length,
    preconditions:['The supplied deployer is an EOA and its next transaction nonce equals startingNonce.','The collection address is the intended deployed IPSEITY collection before TokenModuleRegistry creation.','Predicted creation addresses are unused. Review current chain gas limits, fees and transaction simulations before signing.','Public archive factory nonces are not reserved: recheck the nonce immediately before creating the archive, then verify its exact code/content/length before signing the workbench anchor. Stop and regenerate on any conflict.','This plan neither installs a module nor grants account spending authority.'],
    summary:{transactions:steps.length,externalCreations:steps.filter(step=>step.to===null).length,internalCreations:internalCreations.length,storedBytes:bytes.length,packedBytes:packed.length,chunkCount:archive.chunkReferences.length,uniqueChunks:archive.chunks.length}};
  return {...result,planSha256:digest(canonical(result))};
}

/** Compile from the current source; use the same compiler profile as the original project. */
export function loadModuleArtifacts(output=compile({quiet:true})) {
  const items={};
  for(const name of NAMES) {
    const matches=Object.entries(output.contracts).filter(([,units])=>units[name]);
    if(matches.length!==1)throw Error('Module artifact must have one source: '+name);
    const [source,units]=matches[0],item=units[name];
    items[name]={contractName:name,sourceName:source,abi:item.abi,bytecode:'0x'+item.evm.bytecode.object,deployedBytecode:'0x'+item.evm.deployedBytecode.object,compiler:solc.version(),metadata:{compiler:{version:solc.version().split('+')[0]},settings:{evmVersion:'cancun',optimizer:{enabled:true,runs:800},viaIR:true}}};
  }
  return items;
}
export async function prepareModuleDeployment(input,{workbenchManifest=path.join(root,'dist/modules/manifest.json')}={}) {
  const artifacts=loadModuleArtifacts(),workbench=readWorkbenchBuild(workbenchManifest),rebuilt=await buildWorkbench({output:null});
  if(!workbench.bytes.equals(rebuilt.bytes)||!workbench.packed.equals(rebuilt.packed)||canonical(workbench.manifest)!==canonical(rebuilt.manifest))throw Error('Workbench differs from current reproducible source build');
  return planModuleDeployment(input,{artifacts,workbench,verification:{scope:'current compiled IPSEITY source and independently rebuilt workbench',compiler:solc.version()}});
}
export async function verifyModuleDeploymentPlan(plan,options={}) {
  if(plan?.schema!=='ipseity.module-system-deployment/1')throw Error('Unsupported module system deployment plan');
  ...
  if(canonical(plan)!==canonical(current))throw Error('Saved module plan differs from current source/workbench');
  return current;
}
if(process.argv[1]&&path.resolve(process.argv[1])===path.resolve(import.meta.filename)) {
  const args=process.argv.slice(2);
  if(args.length===1&&args[0]==='--help')console.log('Unsigned only: node tools/modules-deployment.mjs config.json NEW_PLAN.json | --verify PLAN.json. Config: chainId, deployer, startingNonce, collection, premises.');
  else if(args.length===2&&args[0]==='--verify') {await verifyModuleDeploymentPlan(JSON.parse(fs.readFileSync(args[1],'utf8')));console.log('Plan matches current source and workbench; no RPC, signing or broadcast.');}
  else if(args.length===2) {const plan=await prepareModuleDeployment(JSON.parse(fs.readFileSync(args[0],'utf8')));fs.writeFileSync(args[1],JSON.stringify(plan,null,2)+'\n',{flag:'wx'});console.log(JSON.stringify({plan:args[1],transactions:plan.steps.length,planSha256:plan.planSha256}));}
  else throw Error('Use --help for unsigned deployment planning.');
}
