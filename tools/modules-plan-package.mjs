...
const HELP=`Prepare unsigned calls for an already packaged IPSEITY module:
  node tools/modules-plan-package.mjs --package DIR --rpc HTTPS_RPC --chain ID --factory ADDRESS --publisher ADDRESS --output archive-plan.json [--reuse chunks.json]
  node tools/modules-plan-package.mjs --package DIR --rpc HTTPS_RPC --chain ID --registry ADDRESS --archive ADDRESS --schema 1|2 --output publish-plan.json
...
async function compileChunk(){
 ...
 const sourceName='src/modules/OnchainApp.sol',source=await regular(path.join(ROOT,sourceName),1048576),settings={optimizer:{enabled:true,runs:800},viaIR:true,evmVersion:'cancun',metadata:{bytecodeHash:'ipfs',appendCBOR:true},outputSelection:{'*':{'AppChunk':['abi','evm.bytecode.object','evm.deployedBytecode.object']}}};
