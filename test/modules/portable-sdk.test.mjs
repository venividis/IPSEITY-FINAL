...
import {buildModuleSDK} from '../../tools/build-module-sdk.mjs';
...
test('npm distributable SDK imports outside the source tree with no installed dependencies and verifies exact module bytes',async t=>{
  ...
  const installed=path.join(base,'node_modules/@ipseity/modules');fs.mkdirSync(path.join(installed,'dist'),{recursive:true});
  const packageFile=path.join(root,'packages/modules/package.json'),pkg=JSON.parse(fs.readFileSync(packageFile));
  assert.equal(pkg.name,'@ipseity/modules');assert.equal(pkg.version,'1.0.0');assert.equal(pkg.dependencies,undefined);
  ...
  const proof=`
import assert from 'node:assert/strict';
import {packageFiles,verifyArchive,parseManifest,canonicalManifest,manifestHash,sha256,releaseInput,releaseIdFor,moduleKeyFor,descriptorFromManifest} from '@ipseity/modules';
import {sha256 as coreHash} from '@ipseity/modules/core';
...
await assert.rejects(()=>import('@ipseity/modules/deployment'),/not defined by "exports"/);
console.log(JSON.stringify({package:'@ipseity/modules',releaseId,manifestHash:packed.manifestHash,verifiedBytes:recovered.files[0].bytes.length}));
`;
  ...
  const result=JSON.parse(child.stdout);assert.equal(result.package,'@ipseity/modules');assert.equal(result.verifiedBytes,32);
});
