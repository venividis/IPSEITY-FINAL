...
contract OnchainApp {
    ...
    constructor(address[] memory chunks_,bytes32 expectedSha){
        ...
        // Hash the assembled bytes in place. Solidity's sha256(bytes) path can
        // allocate another complete input buffer; at 1 MiB its quadratic memory
        // cost prevents a maximum-size cartridge retaining wallet gas headroom.
        bytes32 digest;bool hashed;
        assembly ("memory-safe") {
            hashed := staticcall(gas(), 2, add(full, 32), mload(full), 0, 32)
            hashed := and(hashed, eq(returndatasize(), 32))
            digest := mload(0)
        }
        require(hashed && digest==expectedSha,"SHA256_MISMATCH");contentSha256=expectedSha;
    }
