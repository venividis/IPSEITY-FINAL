...
contract DeskTrade {
    ...
    string internal constant SWAP_JS =
        ...
        "const receipt=async h=>{const p=I.pv();if(!p)throw new Error('wallet disconnected');"
        "for(let n=0;n<240;n++){const r=await p.request({method:'eth_getTransactionReceipt',params:[h]});"
        "if(r){if(BigInt(r.status||0)!==1n)throw new Error('swap reverted');return r}"
        "await new Promise(ok=>setTimeout(ok,n<10?250:3000))}"
        "throw new Error('swap is still pending; check your wallet before trying again')};"
        "const after=(amt)=>{const p=$('after');if(!p||!B)return;p.hidden=false;"
        "put('afterSum','Acquired about '+I.fmt(amt,B.d,8)+' '+B.s+"
        "'. Give the asset a future, or give the block a meaning.');"
        "try{localStorage.setItem('ipse.flow.lock',JSON.stringify({token:B.a,amount:amt.toString(),"
        "symbol:B.s,decimals:B.d,at:Date.now()}))}catch(e){}"
        "const a=$('afterLock');if(a)a.href='/lock';p.scrollIntoView&&p.scrollIntoView({behavior:'smooth'});};"
        ...
        "const br=await I.tryCall(B.a,S.balanceOf+I.AD(I.acct()));"
        "const before=br?I.word(br,0):0n;"
        "const h=await I.send(U.router,build(amt,minOut,b.fee));"
        "I.say('waiting for swap confirmation \u2026');await receipt(h);"
        "const ar=await I.tryCall(B.a,S.balanceOf+I.AD(I.acct()));"
        "if(!ar)throw new Error('swap confirmed, but the received balance could not be read');"
        "const now=I.word(ar,0);if(now<before)throw new Error('swap confirmed, but the received balance decreased');"
        "after(now-before);"
        "}catch(e){I.say(String(e&&e.message||e),'no')}});"
        "const tell=$('afterTell');if(tell)tell.addEventListener('click',()=>{"
        "const note=String(($('memory')&&$('memory').value)||'').trim();"
        "if(!note){I.say('write the memory first','no');return}"
        "try{localStorage.setItem('ipse.flow.memory',JSON.stringify({"
        "text:'TRADE MEMORY \\u00b7 '+note,token:B?B.a:'',symbol:B?B.s:'',at:Date.now()}))}catch(e){}"
        "location.href='/chat'});"
        "paint();refresh();"
