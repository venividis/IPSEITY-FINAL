...
(function () {
  ...
  function propose(title, lines, tx, fn) {
    ...
    /*  The wrong chain refuses HERE, before a slab exists — §E.5. The
        ...
        a trap with a countdown. The signing press also re-reads chain and
        account: a cached answer cannot bind a wallet that moved later.  */
    if (C.chainOk === false) {
      return say("Your wallet is on another chain. This token lives on chain " +
                 C.chain + " — the crest offers the move.", "err");
    }
    var reviewedAccount = C.account;
    var reviewedProvider = provider();
    var reviewedEpoch = C.walletEpoch || 0;
    var sending = false;
    ...
    put("Chain", String(C.chain));
    put("From", short(reviewedAccount));
    put("To", short(tx.to));
    ...
    slab.querySelector("[data-go]").onclick = function () {
      ...
      shut();
      var p = provider();
      if (!p || !reviewedAccount) return say("Connect a wallet first.", "err");
      say("Checking the wallet before signing…");
      Promise.all([
        p.request({ method: "eth_chainId" }),
        p.request({ method: "eth_accounts" })
      ]).then(function (answers) {
        if (p !== reviewedProvider || p !== provider() ||
            reviewedEpoch !== (C.walletEpoch || 0) ||
            !answers[1] || !answers[1][0] ||
            answers[1][0].toLowerCase() !== reviewedAccount.toLowerCase() ||
            !C.account || C.account.toLowerCase() !== reviewedAccount.toLowerCase()) {
          throw new Error("The wallet changed. Review the action again.");
        }
        if (BigInt(answers[0]) !== BigInt(C.chain)) {
          C.chainOk = false;
          throw new Error("This token lives on chain " + C.chain +
                          ". Move the wallet there, then review the action again.");
        }
        say("Waiting for the wallet…");
        return p.request({ method: "eth_sendTransaction", params: [Object.assign({}, tx, {
          from: reviewedAccount, chainId: "0x" + BigInt(C.chain).toString(16)
        })] });
      })
        .then(function (h) {
