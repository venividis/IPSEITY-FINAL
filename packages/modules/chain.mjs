...
export const ACCOUNT_ABI=['function execute(address target,uint256 value,bytes data,uint8 operation) payable returns(bytes)','function owner() view returns(address)','function token() view returns(uint256 chainId,address collection,uint256 tokenId)','function state() view returns(uint256)','function isSealed() view returns(bool)'];
export const COLLECTION_ABI=['function ownerOf(uint256) view returns(address)','function account(uint256) view returns(address)','function statsOf(uint256) view returns(uint256 ops,uint256 xfers,uint256 strata,uint256 open)'];
export const AUTHORITY_TYPE=keccak256(toUtf8Bytes('ipseity.reach-modules/1'));
...
export function accountCall({chainId,owner,account,target,data,description,preconditions={}}){checkedHex(data,2*1024*1024);return unsignedCall({chainId,from:owner,to:account,abi:ACCOUNT_ABI,method:'execute',args:[address(target),0,data,0],description,preconditions});}
...
export async function readRegistry({request,registry,tokenId,chainId,block,snapshot,cursor=0,historyCursor=0,limit=64}){
 ...
 const [authorityType]=await read('authorityType');if(authorityType!==AUTHORITY_TYPE)throw Error('Registry does not use the IPSEITY Reach authority model');
 ...
 const [account]=await readCall(request,snapshot,collection,COLLECTION_ABI,'account',[tokenId]),[owner]=await readCall(request,snapshot,collection,COLLECTION_ABI,'ownerOf',[tokenId]);
 const [currentOwner]=await readCall(request,snapshot,account,ACCOUNT_ABI,'owner'),[actionNonce]=await readCall(request,snapshot,account,ACCOUNT_ABI,'state'),[sealed]=await readCall(request,snapshot,account,ACCOUNT_ABI,'isSealed');
 const [accountChain,accountCollection,accountToken]=await readCall(request,snapshot,account,ACCOUNT_ABI,'token',[],96);
 const [,transfers]=await readCall(request,snapshot,collection,COLLECTION_ABI,'statsOf',[tokenId],128);
 if(accountChain!==BigInt(snapshot.chainId)||address(accountCollection)!==address(collection)||accountToken!==BigInt(tokenId))throw Error('Account token binding differs from the selected IPSEITY');
 if(transfers<0n||transfers>0xffffffffn)throw Error('Invalid IPSEITY custody counter');
 const epoch=transfers+1n,mode=0,custodyExhausted=transfers===0xffffffffn;
...
function tokenRecipe(context,method,args,description){if(context.custodyExhausted)throw Error('IPSEITY custody counter exhausted; recovery remains available but writes are refused');if(context.mode!==0)throw Error('This recipe requires the IPSEITY Reach owner context');return accountCall({chainId:context.chainId,owner:context.owner,account:context.account,target:context.registry,data:iface(TOKEN_MODULE_ABI).encodeFunctionData(method,args),description,preconditions:contextChecks(context)});}
...
export async function assertCurrentContext({request,context}){const current=await readRegistry({request,registry:context.registry,tokenId:context.tokenId,chainId:context.chainId,limit:1});for(const field of ['collection','account','owner','epoch','root','actionNonce','mode','sealed','authorityType','custodyExhausted'])if(String(current[field]).toLowerCase()!==String(context[field]).toLowerCase())throw Error('Stale review: '+field+' changed');return current;}
